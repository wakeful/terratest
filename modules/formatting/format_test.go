...
func TestFormatPluginDirAsArgs(t *testing.T) {
	...
	for _, testCase := range testCases {
		checkResultWithRetry(t, 100, testCase.expected, fmt.Sprintf("FormatPluginDirAsArgs(%v)", testCase.dir), func() interface{} {
			return FormatPluginDirAsArgs(testCase.dir)
		})
	}
}
...
func TestFormatBackendConfigAsArgs(t *testing.T) {
	...
	testCases := []struct {
		...
		{map[string]interface{}{"foo": "bar"}, []string{"-backend-config=foo=bar"}},
		{map[string]interface{}{"foo": 123}, []string{"-backend-config=foo=123"}},
		{map[string]interface{}{"foo": true}, []string{"-backend-config=foo=true"}},
		{map[string]interface{}{"foo": nil}, []string{"-backend-config=foo"}},
		{map[string]interface{}{"foo": []int{1, 2, 3}}, []string{"-backend-config=foo=[1, 2, 3]"}},
		{map[string]interface{}{"foo": map[string]string{"baz": "blah"}}, []string{"-backend-config=foo={\"baz\" = \"blah\"}"}},
		{
			map[string]interface{}{"str": "bar", "int": -1, "bool": false, "list": []string{"foo", "bar", "baz"}, "map": map[string]int{"foo": 0}},
			[]string{"-backend-config=str=bar", "-backend-config=int=-1", "-backend-config=bool=false", "-backend-config=list=[\"foo\", \"bar\", \"baz\"]", "-backend-config=map={\"foo\" = 0}"},
		},
	}

	for _, testCase := range testCases {
		checkResultWithRetry(t, 100, testCase.expected, fmt.Sprintf("FormatBackendConfigAsArgs(%v)", testCase.vars), func() interface{} {
			return FormatBackendConfigAsArgs(testCase.vars)
		})
	}
}

func TestFormatTerraformArgs(t *testing.T) {
	t.Parallel()
	...
	for _, testCase := range testCases {
		checkResultWithRetry(t, 100, testCase.expected, fmt.Sprintf("FormatTerraformArgs(%v)", testCase.vars), func() interface{} {
			return FormatTerraformArgs(testCase.vars, "-var", true, false)
		})
	}
}
